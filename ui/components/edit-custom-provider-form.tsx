...
import { useUpdateCustomProvider } from '@/lib/hooks/useProviders';
...
interface EditCustomProviderFormProps {
  provider: Provider;
  onSuccess?: () => void;
  onCancel?: () => void;
}
...
export function EditCustomProviderForm({
  ...
  const [useOnion, setUseOnion] = useState(provider.use_onion);
  const [mintInput, setMintInput] = useState('');
  const [mints, setMints] = useState<string[]>(provider.mints);
  ...
  useEffect(() => {
    ...
    setUrl(provider.url);
    setUseOnion(provider.use_onion);
    setMints(provider.mints);
    setMintInput('');
  }, [provider]);
  ...
  return (
    <Card className='w-full max-w-2xl'>
      <CardHeader>
        ...
        <CardDescription>
          Update your custom Nostr marketplace provider settings
        </CardDescription>
      </CardHeader>
      <CardContent>
