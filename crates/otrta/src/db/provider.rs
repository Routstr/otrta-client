...
pub async fn get_all_providers(db: &Pool) -> Result<Vec<Provider>, sqlx::Error> {
    let providers = sqlx::query_as::<_, Provider>(
        "SELECT id, name, url, mints, use_onion, followers, zaps, is_default, created_at, updated_at 
         FROM providers 
         ORDER BY is_default DESC, followers DESC, zaps DESC"
    )
    .fetch_all(db)
    .await?;
...
pub async fn upsert_provider(
    ...
    let provider = sqlx::query_as::<_, Provider>(
        "INSERT INTO providers (name, url, mints, use_onion, followers, zaps, updated_at)
         ...
         DO UPDATE SET 
            ...
            zaps = EXCLUDED.zaps,
            updated_at = NOW()
         RETURNING id, name, url, mints, use_onion, followers, zaps, is_default, created_at, updated_at"
    )
    .bind(name)
    .bind(url)
    .bind(&mints)
...
pub async fn refresh_providers_from_nostr(db: &Pool) -> Result<RefreshProvidersResponse, Box<dyn std::error::Error>> {
    ...
    // Simulate fetching updated data from Nostr
    ...
    for (id, new_followers, new_zaps) in mock_updates {
        match sqlx::query(
            "UPDATE providers SET followers = $1, zaps = $2, updated_at = NOW() WHERE id = $3"
        )
        .bind(new_followers)
        .bind(new_zaps)
        .bind(id)
